...
const NAV_ITEMS: NavItem[] = [
  { id: 'home',     label: 'Today',     sub: 'at a glance',    route: '/',         k: 'H', icon: '◎' },
  { id: 'compose',  label: 'Write',     sub: 'new memory',     route: '/compose',  k: 'W', icon: '✍' },
  { id: 'history',  label: 'Timeline',  sub: 'by date',        route: '/history',  k: 'T', icon: '◷' },
  { id: 'library',  label: 'Memories',  sub: 'all entries',    route: '/library',  k: 'M', icon: '▦' },
  { id: 'reflect',  label: 'Reflect',   sub: 'your patterns',  route: '/reflect',  k: 'R', icon: '◈' },
  { id: 'chat',     label: 'Ask',       sub: 'your past self', route: '/chat',     k: 'C', icon: '◉' },
  { id: 'settings', label: 'Settings',  sub: 'preferences',    route: '/settings', k: 'S', icon: '⚙' },
];

const AUTH_ROUTES = ['/login', '/register', '/shared'];
...
export class AppComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.router.events.pipe(
      ...
      else if (url.startsWith('/history'))    this.activeRoute.set('history');
      else if (url.startsWith('/library'))    this.activeRoute.set('library');
      ...
      else if (url.startsWith('/entry'))      this.activeRoute.set('history');
  ...
  crumb(): string {
    const map: Record<string, string> = {
      home: 'Today', compose: 'New entry', history: 'Timeline',
      library: 'Memories', reflect: 'Reflect', chat: 'Ask', settings: 'Settings',
    };
  ...
  private static readonly DAY_ORBS = [
    { hue: 190, warmth: 0.52 }, // Sunday   — soft teal
    { hue: 56,  warmth: 0.72 }, // Monday   — gold
    { hue: 28,  warmth: 0.78 }, // Tuesday  — warm orange
    { hue: 130, warmth: 0.58 }, // Wednesday — green
    { hue: 280, warmth: 0.52 }, // Thursday — purple
    { hue: 42,  warmth: 0.82 }, // Friday   — amber
    { hue: 340, warmth: 0.62 }, // Saturday — rose
  ];
  ...
  onKey(e: KeyboardEvent) {
    ...
    if (e.key === 't' || e.key === 'T') this.nav('/history');
    if (e.key === 'm' || e.key === 'M') this.nav('/library');
